...
public class Ast {
    ...
    sealed public interface Expression permits Assignment, BinaryExpression, BlockExpression, Identifier, IfExpression, IntegerLiteral, WhileExpression {
    }
    ...
    public static IfExpression If(
            Expression condition, Expression thenClause
    ) {
    ...
    public static final record BinaryExpression(
            Operator operator, Expression lhs, Expression rhs) implements Expression {
    }
    ...
    public final static record BlockExpression(
            List<Expression> elements
    ) implements Expression {
    }

    public final static record WhileExpression(
            Expression condition,
            Expression body
    ) implements Expression {
    }

    public final static record IfExpression(
            Expression condition,
    ...
    //// Environment
    public final static record Environment(
            Map<String, Integer> bindings, Optional<Environment> next
    ) {

        /**
         * パラメータ名を Environment の中から探す。
         * bindings から name が存在するかどうかをチェック。なければ next で findBindings を実行
